 ...
 * Наприклад:
 * userObj.firstName ← 'Max'
 * userObj.lastName  ← 'Kyrylchuk'
 * userObj.fullName() → 'Max Kyrylchuk'.
 */
...
console.log(userObj.fullName()) // Max Kyrylchuk
...
function defUpperStr(text = "DEFAULT TEXT") {
    return text.toUpperCase(); // або return text ? text.toUpperCase() : "DEFAULT TEXT";
}
...
function ageClassification(n) {
    const result =  n > 0 && n <= 24 ? 'Дитинство' :
                    n > 24 && n <= 44 ? 'Молодість' :
                    n > 44 && n <= 65 ? 'Зрілість' :
                    n > 65 && n <= 75 ? 'Старість' :
